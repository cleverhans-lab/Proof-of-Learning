...
def verify_topq(dir, lr, batch_size, dataset, architecture, save_freq, order, threshold, q=10, half=0):
    if not os.path.exists(dir):
        raise FileNotFoundError("Model directory not found")
    sequence = np.load(os.path.join(dir, "indices.npy"))
    ...
    next_model = os.path.join(dir, f"model_step_0")
    for i in range(0, sequence.shape[0], save_freq):
        current_model = next_model
        if i + save_freq >= sequence.shape[0]:
            next_model = os.path.join(dir, f"model_step_{sequence.shape[0]}")
        else:
            next_model = os.path.join(dir, f"model_step_{i + save_freq}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--delta', type=float, default=[1000, 10, 0.1, 0.001],
